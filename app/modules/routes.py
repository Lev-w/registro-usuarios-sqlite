...
def crear_usuario():
    ...
    try:
        db.crear_usuario(**datos)
    except sqlite3.IntegrityError:
        abort(400, description="El usuario ya existe")

    return jsonify(responses.success_response("Usuario agregado.")), 201
...
def login_route():
    ...
    usuario = usuario_service.login(datos)
    ...
    return jsonify(responses.success_response(session["username"], "Login completado.")), 200
...
def logout():
    session.clear()
    db.registrar_accion(
        actor_id=g.usuario["id"],
        accion="LOGOUT",
        entidad="usuario"
    )
    return jsonify(responses.success_response("Sesion cerrada")), 200
...
def cambiar_rol(id):
    ...
    return jsonify(responses.success_response("Rol actualizado")), 200
...
def ver_auditoria():
    ...
    logs = auditoria_service.obtener_logs(filtros, orden, limit, offset)
